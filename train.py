...
from models.fdta.id_criterion import build as build_id_criterion
...
def train_engine(config: dict):
    ...
    # Init Logger:
    logger = Logger(logdir=os.path.join(outputs_dir, "train"))
    ...
    # Build FDTA model:
    model, detr_criterion = build_fdta(config=config)
    ...
    if config["DETR_NUM_TRAIN_FRAMES"] == 0:
        for n, p in model.named_parameters():
            if "detr" in n:
                p.requires_grad = False     # only train the FDTA part.
    param_groups = get_param_groups(model, detr_criterion, config)
    optimizer = AdamW(
        params=param_groups,
        lr=config["LR"],
...
def train_one_epoch(
    ...
    for step, samples in enumerate(dataloader):
        images, depthmaps, annotations, metas = samples["images"], samples['depthmaps'], samples["annotations"], samples["metas"]
        ...
        #deal with depthmaps
        depthmaps.tensors = v2.functional.to_dtype(depthmaps.tensors, dtype=torch.float32, scale=False)
        depthmaps.tensors = depthmaps.tensors * (~depthmaps.mask[:, :, None, ...]).to(torch.float32)
        depthmaps.tensors = depthmaps.tensors.contiguous()
        ...
        # Select the training and no_grad depthmaps:
        detr_train_depthmaps = nested_tensor_index_select(depthmaps, dim=1, index=detr_train_frame_idxs)
        detr_no_grad_depthmaps = nested_tensor_index_select(depthmaps, dim=1, index=detr_no_grad_frame_idxs)
        ...
        #Prepare for the DETR forward function, turn the (B, T, ...) depthmaps to (B*T, ...) (or said flatten):
        detr_train_depthmaps.tensors = einops.rearrange(detr_train_depthmaps.tensors, "b t c h w -> (b t) c h w").contiguous()
        detr_train_depthmaps.mask = einops.rearrange(detr_train_depthmaps.mask, "b t h w -> (b t) h w").contiguous()
        detr_no_grad_depthmaps.tensors = einops.rearrange(detr_no_grad_depthmaps.tensors, "b t c h w -> (b t) c h w").contiguous()
        detr_no_grad_depthmaps.mask = einops.rearrange(detr_no_grad_depthmaps.mask, "b t h w -> (b t) h w").contiguous()
        ...
        if _T > detr_num_train_frames:      # do have no_grad frames (if not, skip this part)
            with torch.no_grad():
                if detr_num_checkpoint_frames == 0 or detr_num_checkpoint_frames * 4 >= len(detr_no_grad_frames):
                    ...
                    detr_no_grad_outputs = model(frames=detr_no_grad_frames, depthmaps=detr_no_grad_depthmaps, part="detr")
                else:
                    ...
                    for batch_samples in batch_iterator(
                        ...
                        detr_no_grad_frames,
                        detr_no_grad_depthmaps,
                    ):
                        batch_frames = batch_samples[0]
                        batch_depthmaps = batch_samples[1]
                        _ = model(frames=batch_frames, depthmaps=batch_depthmaps,  part="detr")
        ...
        if detr_num_train_frames > 0:
            if detr_num_checkpoint_frames == 0 or detr_num_checkpoint_frames >= len(detr_train_frames):
                ...
                detr_train_outputs = model(frames=detr_train_frames, depthmaps=detr_train_depthmaps, part="detr")
            else:
                ...
                for batch_samples in batch_iterator(
                    ...
                    detr_train_frames,
                    detr_train_depthmaps,
                ):
                    batch_frames = batch_samples[0]
                    batch_depthmaps = batch_samples[1]
                    _ = model(frames=batch_frames, depthmaps=batch_depthmaps, part="detr", use_checkpoint=True)
        ...
        # DETR criterion:
        detr_loss_dict, detr_indices = detr_criterion(outputs=detr_outputs, targets=detr_targets_flatten, batch_len=detr_criterion_batch_len, epoch=epoch)

        # Whether to only train the DETR, OR to train the FDTA together:
        if not only_detr:
            _G, _, _N = annotations[0][0]["trajectory_id_labels"].shape
            # Need to prepare for FDTA:
            seq_info = prepare_for_fdta(
                detr_outputs=detr_outputs, annotations=annotations, detr_indices=detr_indices,
            )
            seq_info = model(seq_info=seq_info, part="trajectory_embedding")
        ...
        if step % logging_interval == 0:
            ...
            # draw tensorboard (only on main process to avoid duplicate entries)
            if accelerator.is_main_process:
                global_step = states["global_step"]
                if "loss" in metrics:
                    writer.add_scalar('Loss/Total', metrics["loss"].global_average, global_step)
                if "detr_loss" in metrics:
                    writer.add_scalar('Loss/DETR', metrics["detr_loss"].global_average, global_step)
                if "id_loss" in metrics:
                    writer.add_scalar('Loss/ID', metrics["id_loss"].global_average, global_step)
                    
                detr_loss_keys = ['loss_ce', 'loss_bbox', 'loss_giou', 'class_error', 'cardinality_error', 'loss_contrastive', 'loss_depth_map']
                for key in detr_loss_keys:
                    if key in metrics:
                        writer.add_scalar(f'DETR_Loss/{key}', metrics[key].global_average, global_step)
                
                if "detr_grad_norm" in metrics:
                    writer.add_scalar('Training/DETR_Grad_Norm', metrics["detr_grad_norm"].global_average, global_step)
                if "other_grad_norm" in metrics:
                    writer.add_scalar('Training/Other_Grad_Norm', metrics["other_grad_norm"].global_average, global_step)
...
def get_param_groups(model, criterion, config) -> list[dict]:
    ...
    param_groups = [
        ...
        },
        {
            "params": [p for n, p in model.named_parameters()
                       ...
                       and p.requires_grad] + list(criterion.parameters())
        }
    ]
    return param_groups
...
def tensor_dict_cat(tensor_dict1, tensor_dict2, dim=0):
    if tensor_dict1 is None or tensor_dict2 is None:
        ...
        for k in tensor_dict1.keys():
            ...
            elif isinstance(tensor_dict1[k], NestedTensor):
                assert isinstance(tensor_dict2[k], NestedTensor), "Both tensor dicts should have the same type."
                new_tensors = torch.cat([tensor_dict1[k].tensors, tensor_dict2[k].tensors], dim=dim).contiguous()
                new_mask = torch.cat([tensor_dict1[k].mask, tensor_dict2[k].mask], dim=dim).contiguous()
                res_tensor_dict[k] = NestedTensor(tensors=new_tensors, mask=new_mask)
            else:
...
def tensor_dict_index_select(tensor_dict, index, dim=0):
    ...
    for k in tensor_dict.keys():
        ...
        elif isinstance(tensor_dict[k], NestedTensor):
            new_tensors = torch.index_select(tensor_dict[k].tensors, index=index, dim=dim).contiguous()
            new_mask = torch.index_select(tensor_dict[k].mask, index=index, dim=dim).contiguous()
            res_tensor_dict[k] = NestedTensor(tensors=new_tensors, mask=new_mask)
        else:
...
def prepare_for_fdta(detr_outputs, annotations, detr_indices):
    _B, _T = len(annotations), len(annotations[0])
    _G, _, _N = annotations[0][0]["trajectory_id_labels"].shape
