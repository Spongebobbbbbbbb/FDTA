...
def collate_fn(batch):
    images, depthmaps, annotations, metas = zip(*batch)    # (B, T, ...)
    _B = len(batch)
    _T = len(images[0])
    ...
    depthmaps_list = [clip[_] for clip in depthmaps for _ in range(len(clip))]
    nested_tensor_depthmaps = nested_tensor_from_tensor_list(depthmaps_list, size_divisibility=size_divisibility)
    # Reshape the nested tensor:
    nested_tensor_depthmaps.tensors = einops.rearrange(
        nested_tensor_depthmaps.tensors, "(b t) c h w -> b t c h w", b=_B, t=_T
    )
    nested_tensor_depthmaps.mask = einops.rearrange(
        nested_tensor_depthmaps.mask, "(b t) h w -> b t h w", b=_B, t=_T
    )
    # Above is prepared for DETR.
    # Below is prepared for FDTA, pre-padding the annotations:
    ...
    return {
        ...
        "depthmaps": nested_tensor_depthmaps,
