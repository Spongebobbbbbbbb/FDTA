...
class JointDataset(Dataset):
    ...
    def __getitem__(self, info):
        ...
        for frame_idx in frame_idxs:
            image_path = self.image_paths[dataset][split][sequence][frame_idx]
            depth_path = image_path.replace("/img1/", "/depth/").replace(".jpg", ".png")
        ...
        depthmaps = [
            Image.open(depth_path) for depth_path in depth_paths
        ]
        ...
        if self.transforms is not None:
            images, depthmaps, annotations, metas = self.transforms(images, depthmaps, annotations, metas)
