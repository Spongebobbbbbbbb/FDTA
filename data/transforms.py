...
class MultiCompose:
    ...
    def __call__(self, images, depthmaps, annotations, metas):
        for transform in self.transforms:
            images, depthmaps, annotations, metas = transform(images, depthmaps, annotations, metas)
...
class MultiSimulate:
    ...
    def __call__(self, images, depthmaps, annotations, metas):
        if metas[0]["is_static"] is False:
            ...
            # However, as discussed in FDTA Appendix C.2, we need more advanced methods to simulate a video clip.
            ...
            shifted_images, shifted_depthmaps, shifted_annotations = [], [], []
            shifted_images.append(copy.deepcopy(images[0]))
            shifted_depthmaps.append(copy.deepcopy(depthmaps[0]))
            ...
            for _idx in range(1, len(images)):
                ...
                _depthmap = copy.deepcopy(shifted_depthmaps[_idx - 1])
                ...
                _depthmap = v2.functional.crop(_depthmap, _i, _j, _h, _w)
                ...
                _depthmap = v2.functional.resize(_depthmap, [h, w])
                # Put into the shifted sequence:
                shifted_images.append(_image)
                shifted_depthmaps.append(_depthmap)
            ...
            if not _is_legals.all().item():
                ...
                if random.random() < 0.5:
                    return shifted_images, shifted_depthmaps, shifted_annotations, metas
                else:
                    # Inverse the sequence:
                    shifted_images = shifted_images[::-1]
                    shifted_depthmaps = shifted_depthmaps[::-1]
...
class MultiStack:
    ...
    def __call__(self, images, depthmaps, annotations, metas):
        ...
        if isinstance(depthmaps, list):
            if isinstance(depthmaps[0], torch.Tensor):
...
class MultiRandomHorizontalFlip:
    ...
    def __call__(self, images, depthmaps, annotations, metas):
        ...
        if torch.rand(1).item() < self.p:
            ...
            if isinstance(depthmaps, torch.Tensor):
                depthmaps = v2.functional.horizontal_flip_image(depthmaps)
            elif isinstance(depthmaps, list):
                assert isinstance(depthmaps[0], Image.Image)
                depthmaps = [v2.functional.hflip(_) for _ in depthmaps]
            else:
                raise NotImplementedError(f"The input depthmap type {type(depthmaps)} is not supported.")
...
class MultiRandomSelect:
    ...
    def __call__(self, images, depthmaps, annotations, metas):
        if torch.rand(1).item() < self.p:
            return self.transform1(images, depthmaps, annotations, metas)
        else:
            return self.transform2(images, depthmaps, annotations, metas)
...
class MultiRandomResize:
    ...
    def __call__(self, images, depthmaps, annotations, metas):
        ...
        #Resize depthmaps:
        if isinstance(depthmaps, torch.Tensor):
            depthmaps = v2.functional.resize(depthmaps, new_hw)
        elif isinstance(depthmaps, list):
            assert isinstance(depthmaps[0], Image.Image)
            depthmaps = [v2.functional.resize(_, new_hw) for _ in depthmaps]
        else:
            raise NotImplementedError(f"The input depthmap type {type(depthmaps)} is not supported.")
        # Resize annotations:
        for annotation in annotations:
...
class MultiRandomCrop:
    ...
    def __call__(self, images, depthmaps, annotations, metas):
        ...
        if not _is_legals.all().item():
            ...
            if isinstance(depthmaps, torch.Tensor):
                depthmaps = v2.functional.crop(depthmaps, _i, _j, _h, _w)
            elif isinstance(depthmaps, list):
                assert isinstance(depthmaps[0], Image.Image)
                depthmaps = [v2.functional.crop(_, _i, _j, _h, _w) for _ in depthmaps]
            else:
                raise NotImplementedError(f"The input depthmap type {type(depthmaps)} is not supported.")
...
class MultiToTensor:
    ...
    def __call__(self, images, depthmaps, annotations, metas):
        ...
        if isinstance(depthmaps, list):
            assert isinstance(depthmaps[0], Image.Image)
            depthmaps = [v2.functional.to_image(_) for _ in depthmaps]
        return images, depthmaps, annotations, metas
...
class MultiNormalizeBoundingBoxes:
    ...
    def __call__(self, images, depthmaps, annotations, metas):
        ...
        for annotation in annotations:
            annotation["bbox"] = annotation["bbox"] / torch.tensor([w, h, w, h])
        return images.contiguous(), depthmaps.contiguous(), annotations, metas


# For FDTA only, biding the ID label:
