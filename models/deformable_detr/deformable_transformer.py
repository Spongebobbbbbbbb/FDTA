...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, depth_pos_embed=None, depth_pos_embed_ip=None):
        ...
        mask_depth = masks[0].flatten(1)

        # decoder
        hs, inter_references = self.decoder(tgt = tgt,
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index, src_padding_mask=None ,depth_pos_embed=None,
                mask_depth=None, depth_pos_embed_ip=None):
        # self attention
        q = k = self.with_pos_embed(tgt, query_pos)
        tgt2 = self.self_attn(q.transpose(0, 1), k.transpose(0, 1), tgt.transpose(0, 1), need_weights=False)[0].transpose(0, 1)
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
                query_pos=None, src_padding_mask=None ,depth_pos_embed=None, mask_depth=None, depth_pos_embed_ip=None):
        output = tgt
