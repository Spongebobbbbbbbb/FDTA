...
class IDDecoder(nn.Module):
    ...
    def forward(self, seq_info, use_decoder_checkpoint):
        """Forward pass for ID prediction.

        Args:
            seq_info: Dictionary containing sequence information for ID prediction,
                including known trajectory information and current unknown detections.
            use_decoder_checkpoint: Whether to use gradient checkpointing for memory efficiency.

        Returns:
            Tuple of (id_logits, id_labels, id_masks) for ID prediction results.
        """
        ...
        # G represents the number of augmentation groups for the same sample
        ...
        # Self-attention mask: for interactions between objects within the same frame
        self_attn_key_padding_mask = einops.rearrange(unknown_masks, "b g t n -> (b g t) n").contiguous()
        # Cross-attention key padding mask: for trajectory-based attention
        ...
        # Temporal causal mask: ensures only historical information is visible
        ...
        # Relative position encoding indices
