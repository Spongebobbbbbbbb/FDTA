...
from models.fdta.trajectory_modeling import TrajectoryModeling
from models.fdta.id_decoder import IDDecoder
...
def build(config: dict):
    ...
    detr_args.depth_loss_coef = config["DETR_DEPTH_LOSS_COEF"]
    detr_args.bbox_loss_coef = config["DETR_BBOX_LOSS_COEF"]
    detr_args.giou_loss_coef = config["DETR_GIOU_LOSS_COEF"]
    detr_args.contrastive_loss_coef = config["DETR_CONTRASTIVE_LOSS_COEF"]
    ...
    detr_args.depth_mode = config["DEPTH_MODE"]
    detr_args.depth_max = config["DEPTH_MAX"]
    detr_args.depth_min = config["DEPTH_MIN"]
    detr_args.depth_bin_num = config["DEPTH_BIN_NUM"]
    detr_args.ddn_fg_weight = config["DEPTH_FG_WEIGHT"]
    ...
    # 1. trajectory embedding selfattention:
    _trajectory_embedding = History_motion_embedding()
